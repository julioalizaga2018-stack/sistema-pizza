<?php
// controllers/ProductoController.php (Parte 1 de 2 - Jungle POS Controller Engine)
    ...
    public function procesarPeticiones() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($accion === 'crear_producto' || $accion === 'editar_producto') {
                if (session_status() === PHP_SESSION_NONE) { session_start(); } // Aseguramos lectura de sesión
...
// controllers/ProductoController.php (Parte 2 de 2 - Bloque de Control e Inyección)
                ...
                // 🌟 REFACTORIZACIÓN CON OJO CLÍNICO: Protección de stock inteligente por Roles VIP
                $rolSesion = isset($_SESSION['rol_id']) ? (int)$_SESSION['rol_id'] : 0;
                $es_usuario_gerencial = ($rolSesion === 1 || $rolSesion === 2); // 1 = superadmin, 2 = admin según phpMyAdmin

                if ($accion === 'editar_producto' && $id > 0) {
                    if ($es_usuario_gerencial) {
                        // Acceso Autorizado: Leemos de forma directa lo que el Administrador digitó en la pantalla
                        $stock_actual = floatval($_POST['stock_actual'] ?? 0);
                    } else {
                        // Acceso Restringido: El input llegó vacío por estar disabled. Activamos tu rescate de base de datos original.
                        $productoActual = $this->modelo->obtenerProductoPorId($id);
                        $stock_actual = $productoActual ? floatval($productoActual['stock_actual']) : 0.00;
                    }
                } else {
                    // Si es un producto totalmente nuevo, sí capturamos el inventario inicial del formulario ordinariamente
                ...
                if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
                    ...
                    // Renombrado único temporal contra problemas de cache
                ...
                // --- ENRUTAMIENTO FINAL ORIGINAL CON INYECCIÓN PERFECTA DE PARÁMETROS EN EL MODELO ---
                if ($accion === 'crear_producto') {
                    // 🌟 $proveedor_id se inyecta estrictamente en la segunda posición
                    ...
                    // 🌟 $proveedor_id se inyecta estrictamente en la tercera posición (Usa la variable rescatada o digitada)
